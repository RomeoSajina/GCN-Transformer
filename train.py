...
def train_one_epoch(config, model, train_loader, optimizer, scheduler):
    ...
    for i, x_orig in enumerate(iter(train_loader)):
        ...
        r = x.reshape(-1, config.N, config.input_len+config.output_len, config.J*3)
...
def train(config):
    ...
    train_loader, valid_loader, test_loader = create_datasets(config=config)
    ...
    for epoch_number in range(1, config.num_epoch+1):
        ...
        vims = calc_vim(config, model, test_loader)
